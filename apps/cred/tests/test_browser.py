...
class JavascriptTests(LiveServerTestCase):
    ...
    def test_search(self):
        ...
        self.selenium.get('%s%s' % (self.live_server_url, reverse('cred:cred_list')))
        self.waitforload()
        ...
        plan_url = urldecode('%s%s' % (self.live_server_url, reverse('cred:cred_list', args=('search', searchkey))))
        self.assertEquals(cur_url, plan_url)

    def test_password_details(self):
        self.login_as(self.data.unorm.username, self.data.normpass)
        self.selenium.get('%s%s' % (self.live_server_url,
            reverse('cred:cred_detail', args=(self.data.cred.id,))))
        self.waitforload()
    ...
    def test_password_edit(self):
        ...
        self.login_as(self.data.unorm.username, self.data.normpass)
        self.selenium.get('%s%s' % (self.live_server_url,
            reverse('cred:cred_edit', args=(self.data.cred.id,))))
        self.waitforload()
    ...
    def test_script_injection(self):
        ...
        self.login_as(self.data.unorm.username, self.data.normpass)
        self.selenium.get('%s%s' % (self.live_server_url,
            reverse('cred:cred_detail', args=(self.data.injectcred.id,))))
        self.waitforload()
        elempass = self.selenium.find_element_by_id('password')
        # Hover over password
        self.selenium.find_elements_by_xpath("//button[contains(concat(' ', @class, ' '), ' btn-pass-fetchcred ')]")[0].click()
