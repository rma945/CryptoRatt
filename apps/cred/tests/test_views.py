...
from apps.cred.models import Cred, Tag
...
class CredViewTests(TestCase):
    ...
    def test_list_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_list'))
    ...
    def test_list_staff(self):
        resp = self.data.staff.get(reverse('cred:cred_list'))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
    ...
    def test_list_trash_normal(self):
        ...
        resp = self.data.norm.get(reverse('cred:cred_list', args=('special', 'trash')))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred not in credlist)
    ...
    def test_list_trash_staff(self):
        ...
        resp = self.data.staff.get(reverse('cred:cred_list', args=('special', 'trash')))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred in credlist)
    ...
    def test_list_changeq_normal(self):
        self.data.cred.delete()
        resp = self.data.norm.get(reverse('cred:cred_list', args=('special', 'changeq')))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred not in credlist)

    def test_list_by_tag_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_list', args=('tag', self.data.tag.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred not in credlist)
    ...
    def test_list_by_tag_staff(self):
        resp = self.data.staff.get(reverse('cred:cred_list', args=('tag', self.data.tag.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred not in credlist)
    ...
    def test_list_by_group_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_list', args=('group', self.data.group.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred in credlist)
        self.assertTrue(self.data.tagcred in credlist)
    ...
    def test_list_by_group_staff(self):
        resp = self.data.staff.get(reverse('cred:cred_list', args=('group', self.data.othergroup.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertTrue(self.data.cred not in credlist)
        self.assertTrue(self.data.tagcred not in credlist)
    ...
    def test_list_by_group_nobody(self):
        resp = self.data.nobody.get(reverse('cred:cred_list', args=('group', self.data.othergroup.id)))
        self.assertEqual(resp.status_code, 404)

    def test_list_by_changeadvice_disable_user(self):
        resp = self.data.staff.get(reverse('cred:cred_list', args=('changeadvice', self.data.unorm.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
    ...
    def test_list_by_changeadvice_user_added(self):
        resp = self.data.staff.get(reverse('cred:cred_list', args=('changeadvice', self.data.unobody.id)))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertIn(self.data.viewedcred, credlist)

    def test_list_by_changeadvice_remove_group(self):
        resp = self.data.staff.get(reverse('cred:cred_list', args=('changeadvice', self.data.unorm.id)) + '?group=%s' % self.data.group.id)
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
        self.assertIn(self.data.viewedcred, credlist)
        self.assertNotIn(self.data.changedcred, credlist)
    ...
    def test_list_by_search_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_list', args=('search', 'tag')))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
    ...
    def test_list_by_search_unicode(self):
        resp = self.data.norm.get(reverse('cred:cred_list', args=('search', 'ö')))
        self.assertEqual(resp.status_code, 200)
        credlist = resp.context['credlist'].object_list
    ...
    def test_detail_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_detail', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['cred'].id, self.data.cred.id)
        self.assertEqual(resp.context['credlogs'], None)
        resp = self.data.norm.get(reverse('cred:cred_detail', args=(self.data.tagcred.id,)))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['cred'].id, self.data.tagcred.id)
        self.assertEqual(resp.context['credlogs'], None)

    def test_detail_markdown(self):
        resp = self.data.norm.get(reverse('cred:cred_detail', args=(self.data.markdowncred.id,)))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_detail_staff(self):
        resp = self.data.staff.get(reverse('cred:cred_detail', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['cred'].id, self.data.cred.id)
        self.assertNotEqual(resp.context['credlogs'], None)
        resp = self.data.staff.get(reverse('cred:cred_detail', args=(self.data.tagcred.id,)))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['cred'].id, self.data.tagcred.id)
        self.assertNotEqual(resp.context['credlogs'], None)

    def test_detail_nobody(self):
        resp = self.data.nobody.get(reverse('cred:cred_detail', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 404)
        resp = self.data.nobody.get(reverse('cred:cred_detail', args=(self.data.tagcred.id,)))
        self.assertEqual(resp.status_code, 404)
    ...
    def test_edit_normal(self):
        resp = self.data.norm.get(reverse('cred:cred_edit', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        form = resp.context['form']
        post = {}
        for i in form:
    ...
    def test_edit_nobody(self):
        resp = self.data.nobody.get(reverse('cred:cred_edit', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 404)

    def test_delete_norm(self):
        resp = self.data.norm.get(reverse('cred:cred_delete', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        resp = self.data.norm.post(reverse('cred:cred_delete', args=(self.data.cred.id,)))
        delcred = Cred.objects.get(id=self.data.cred.id)
        self.assertTrue(delcred.is_deleted)

    def test_delete_staff(self):
        resp = self.data.staff.get(reverse('cred:cred_delete', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        resp = self.data.staff.post(reverse('cred:cred_delete', args=(self.data.cred.id,)), follow=True)
        self.assertEqual(resp.status_code, 200)
        delcred = Cred.objects.get(id=self.data.cred.id)
        self.assertTrue(delcred.is_deleted)

    def test_delete_nobody(self):
        resp = self.data.nobody.get(reverse('cred:cred_delete', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 404)
        resp = self.data.nobody.post(reverse('cred:cred_delete', args=(self.data.cred.id,)), follow=True)
        self.assertEqual(resp.status_code, 404)
        delcred = Cred.objects.get(id=self.data.cred.id)
        self.assertFalse(delcred.is_deleted)

    def test_tagadd_normal(self):
        resp = self.data.norm.get(reverse('cred:tag_add'))
        self.assertEqual(resp.status_code, 200)
        form = resp.context['form']
        post = {}
        for i in form:
            if i.value() is not None:
                post[i.name] = i.value()
        post['name'] = 'New Tag'
        resp = self.data.norm.post(reverse('cred:tag_add'), post, follow=True)
        self.assertEqual(resp.status_code, 200)
        newtag = Tag.objects.get(name='New Tag')
        self.assertTrue(newtag is not None)

    def test_tagedit_normal(self):
        resp = self.data.norm.get(reverse('cred:tag_edit', args=(self.data.tag.id,)))
        self.assertEqual(resp.status_code, 200)
        form = resp.context['form']
        post = {}
        for i in form:
            if i.value() is not None:
                post[i.name] = i.value()
        post['name'] = 'A Tag'
        resp = self.data.norm.post(reverse('cred:tag_edit', args=(self.data.tag.id,)), post, follow=True)
        self.assertEqual(resp.status_code, 200)
        newtag = Tag.objects.get(name='A Tag')
        self.assertEqual(newtag.id, self.data.tag.id)

    def test_tagdelete_normal(self):
        resp = self.data.norm.get(reverse('cred:tag_delete', args=(self.data.tag.id,)))
        self.assertEqual(resp.status_code, 200)
        resp = self.data.staff.post(reverse('cred:tag_delete', args=(self.data.tag.id,)), follow=True)
        self.assertEqual(resp.status_code, 200)
    ...
    def test_viewqueue_nobody(self):
        resp = self.data.nobody.get(reverse('cred:cred_list', args=('special', 'changeq')))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(len(resp.context['credlist']), 0)

    def test_addtoqueuestaff(self):
        resp = self.data.staff.get(reverse('cred:cred_add_to_queue',
            args=(self.data.tagcred.id,)), follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(self.data.tagcred.on_changeq())

    def test_bulkdelete_staff(self):
        resp = self.data.staff.post(reverse('cred:bulk_delete'), {'credcheck': self.data.cred.id}, follow=True)
        self.assertEqual(resp.status_code, 200)
        delcred = Cred.objects.get(id=self.data.cred.id)
        self.assertTrue(delcred.is_deleted)
    ...
    def test_bulkaddtochangeq_staff(self):
        resp = self.data.staff.post(reverse('cred:bulk_add_to_queue'), {'credcheck': self.data.tagcred.id}, follow=True)
        self.assertEqual(resp.status_code, 200)
        self.assertTrue(self.data.tagcred.on_changeq())
    ...
    def test_deeplink_post_login_redirect(self):
        ...
        credurl = reverse('cred:cred_detail', args=(self.data.cred.id,))
