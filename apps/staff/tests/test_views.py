...
class StaffViewTests(TestCase):
    ...
    def test_userdetail(self):
        resp = self.data.staff.get(reverse('staff:user_detail', args=(self.data.unobody.id,)))
        self.assertEqual(resp.status_code, 200)
    ...
    def test_groupadd(self):
        resp = self.data.staff.get(reverse('staff:group_add'))
        self.assertEqual(resp.status_code, 200)
        ...
        resp = self.data.staff.post(reverse('staff:group_add'), post, follow=True)
        self.assertEqual(resp.status_code, 200)
        # Make sure we can get the object without exception
        Group.objects.get(name='Test Group')

    def test_groupdetail(self):
        resp = self.data.staff.get(reverse('staff:group_detail',
    ...
    def test_groupdelete(self):
        resp = self.data.staff.get(reverse('staff:group_delete',
            args=(self.data.othergroup.id,)))
        self.assertEqual(resp.status_code, 200)
        group = resp.context['group']
        self.assertEqual(self.data.othergroup.id, group.id)
        resp = self.data.staff.post(reverse('staff:group_delete',
    ...
    def test_userdelete(self):
        resp = self.data.staff.get(reverse('staff:user_delete',
            args=(self.data.unobody.id,)))
        self.assertEqual(resp.status_code, 200)
        user = resp.context['viewuser']
        self.assertEqual(self.data.unobody.id, user.id)
        resp = self.data.staff.post(reverse('staff:user_delete',
    ...
    def test_credundelete(self):
        ...
        resp = self.data.staff.get(reverse('staff:cred_undelete', args=(self.data.cred.id,)))
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(resp.context['cred'], self.data.cred)
        resp = self.data.staff.post(reverse('staffL:cred_undelete', args=(self.data.cred.id,)), follow=True)
        self.assertEqual(resp.status_code, 200)
